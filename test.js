...
const processedData = processJson(jsonData, "English", "Spanish", input_vocab_size, target_vocab_size, MAX_TOKENS)

const input_language = processedData.trainData.map((item) => item.English)

const output_language = processedData.trainData.map((item) => item.Spanish)
...
const train_x = tf.tensor(input_language.slice(0, batch_size))
const train_y = tf.tensor(output_language.slice(0, batch_size))
